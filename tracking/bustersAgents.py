...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for i in livingGhostPositionDistributions:
            iBestPos = i.argMax()
            iPosDist = self.distancer.getDistance(iBestPos,pacmanPosition)
            if iPosDist < minDist:
                 minDist, closestPos = iPosDist, iBestPos

        minAction = None
        minDist = float('inf')
        for a in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, a)
            dister = self.distancer.getDistance(successorPosition, closestPos)
            if dister < minDist:
                minAction = a
                minDist = dister
        return minAction
